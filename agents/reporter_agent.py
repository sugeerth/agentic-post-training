...
class ReporterAgent(BaseAgent):
    ...
    async def run(self, **kwargs) -> dict[str, Any]:
        ...
        return {"report_md": str(md_path), "report_json": str(json_path), "tldr": tldr}

    async def step(self, **kwargs) -> dict[str, Any]:
        return await self.run(**kwargs)

    def _compose_tldr(self, run: dict[str, Any]) -> str:
        ...
        lines.append(f"- {_next_action(results, anomalies)}")
        lines.append("")
...
def _next_action(results: dict[str, Any], anomalies: list[dict]) -> str:
    if anomalies:
        first = anomalies[0]
        return f"Address `{first['stage']}` — {first['reason']}."
    # No anomalies — pick the weakest signal to improve.
    for _stage, r in results.items():
...
def _slim(obj: Any, depth: int = 0) -> Any:
    ...
    if isinstance(obj, dict):
        return {k: _slim(v, depth + 1) for k, v in obj.items() if not _skip(k, v)}
...
def _skip(key: str, value: Any) -> bool:
    if key in {"all_metrics", "iteration_metrics"} and isinstance(value, list) and len(value) > 3:
        return False  # keep but slim
    return False
